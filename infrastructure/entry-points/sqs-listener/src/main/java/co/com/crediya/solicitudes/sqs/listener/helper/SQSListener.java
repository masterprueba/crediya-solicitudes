...
public class SQSListener {
    ...
    public SQSListener start() {
        ...
        log.info("🚀 Iniciando SQS Listener - Queue: {}, Threads: {}", properties.queueUrl(), properties.numberOfThreads());
        ...
        for (var i = 0; i < properties.numberOfThreads(); i++) {
            log.info("🔄 Iniciando hilo de escucha #{}", i + 1);
            flow.subscribe();
        }
        log.info("✅ SQS Listener iniciado correctamente con {} hilos", properties.numberOfThreads());
        return this;
    }
    ...
    private Flux<Message> getMessages() {
        return Mono.fromCallable(this::getReceiveMessageRequest)
                .doOnNext(request -> log.debug("🔍 Consultando cola SQS: {}", request.queueUrl()))
                .flatMap(request -> Mono.fromFuture(client.receiveMessage(request)))
                .doOnNext(response -> {
                    if (response.messages().isEmpty()) {
                        log.debug("📭 No hay mensajes en la cola");
                    } else {
                        log.info("📬 Recibidos {} mensajes de SQS", response.messages().size());
                    }
                })
